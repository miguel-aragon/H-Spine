...
#include "../inc/water_functions_uphill_07.h"
...
//       watershed_uphill_27
//   DESCRIPTION:
//       This is just a nice wraper for the watershed
//       functions.
//   NOTES:
//==========================================
int *watershed_uphill_07(double *density, int x_grid, int y_grid, int z_grid)
{
  //--- Local minima
  ...
  get_closest(density, closest_minima, x_grid, y_grid, z_grid);

  //--- Flood basins bu going uphill from the local minima.
  flood_basin_uphill_07(closest_minima, voids, x_grid, y_grid, z_grid, n_local_minima, local_minima);
...
//   NOTES:
//==========================================
int *watershed_uphill_07_float(float *density, int x_grid, int y_grid, int z_grid)
{
  //--- Local minima
  int *local_minima=NULL;
  int  n_local_minima=0;
  ...
  //--- Find local minima
  //==================================
  local_minima = find_minima_int_07_float(density, x_grid,y_grid,z_grid, &n_local_minima);
  printf(">>> Ready finding minima [%d]\n", n_local_minima);
  fflush(stdout);
  ...
  //--- Get closest (lower) pixel in neighbourhood
  get_closest_float_07(density, closest_minima, x_grid, y_grid, z_grid);
...
//   NOTES:
//==========================================
void flood_basin_uphill_07(int *closest_minima, int *voids, int x_grid, int y_grid, int z_grid, int total_minima, int *local_minima)
{
  int    i,j,k,w,q;
  //--- Shell
  int    Ind_Shell[7];
  ...
  for(i=0; i<total_minima; i++){
    ...
    decomposed_index3d(xyz, Void_Index, xd_grid,yd_grid,zd_grid);

    //--- Fix bug, check later...
    xg = xyz[0]; yg = xyz[1]; zg = xyz[2]; 

    //--- Get shell around void center
    shell_07(shell_ind, xg,yg,zg, x_grid,y_grid,z_grid);

    if (Void_Index != shell_ind[0]) {
      printf(">>> ERROR!!! %d == %d ?\n", Void_Index, shell_ind[0]);
      printf(">>> %d [%d %d %d] [%f %f %f]\n", i, xg,yg,zg, xd_grid,yd_grid,zd_grid);
      fflush(stdout);
      exit(0);
    }
    ...
    //-----------------------------------------
    recursive_flood_uphill_07(where_array, closest_minima, voids, Void_Id, x_grid, y_grid, z_grid);
...
//   NOTES:
//==========================================
void recursive_flood_uphill_07(int *children, int *closest_minima, int *voids, int ID, int x_grid, int y_grid, int z_grid){
  ...
  for (i=0;i<children[0];i++){
    ...
    //--- Get the 3d indexes of local minima
    decomposed_index3d(xyz, index_i, xd_grid,yd_grid,zd_grid);
    //--- Fix bug, check later...
    xg = xyz[0]; yg = xyz[1]; zg = xyz[2]; 
    ...
    //--- Call recursively
    //-----------------------------------------
    recursive_flood_uphill_07(where_array, closest_minima, voids, ID, x_grid, y_grid, z_grid);
