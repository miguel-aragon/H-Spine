...
#include "../inc/watershed_spine_07.h"
...
char *get_void_boundaries_07(int *voids, int sizeX, int sizeY, int sizeZ)
{
  ...
  for(i=0; i<sizeX; i++){
	...
	shell_07(shell_ind, i,j,k, sizeX,sizeY,sizeZ);
	
	//--- Copy neighbourhood to array
	for(w=0; w<7; w++) shell_array[w] = voids[shell_ind[w]];
...
void local_watershed_sort_float_07(float *den, int *voids, char *boundaries, int sizeX, int sizeY, int sizeZ)
{
  ...
  for (j=0;j<n_where_boundaries;j++) {
    ...
    xg = xyz[0]; yg = xyz[1]; zg = xyz[2]; 
    //--- Get shell around pixel
    shell_07(shell_ind, xg,yg,zg, sizeX,sizeY,sizeZ);
    
    //--- ID's of voids inside shell
    for (i=0;i<7;i++) shell_array[i]=voids[shell_ind[i]];
    ...
    //--- Compute slopes from center pixel
    for (i=0;i<7;i++) slope_shell[i] = den[shell_ind[0]] - den[shell_ind[i]];
...
//==========================================
void count_adjacent_voids_07(int *voids, char *boundaries, int sizeX, int sizeY, int sizeZ)
{
  int  i,j,k,w;
  int  n_shell = 7;
  int  *shell_ind = malloc(n_shell*sizeof(int));  
  ...
  for(i=0; i<sizeX; i++){
	...
	if (voids[shell_ind[0]] == 0) {
	  ...
	  if (region_count ==1) if (shell_array[0] == 0) boundaries[shell_ind[0]] = 255;
